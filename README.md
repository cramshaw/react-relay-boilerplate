Whilst this was originally created with Create React App, it has since been ejected in order to run Relay on it. The app relies on the server: https://github.com/cramshaw/express-graphql-app

In order to run, `yarn start` will get things going.

`yarn relay` is required after any graphql changes, this can be run under watchman with `yarn relay --watch`.

It is intended to be an 'as simple as possible' relay app.
